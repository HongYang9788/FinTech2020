class Strategy():
    ...
    def trade(self, information):
        ...
        if self.first_buy:
            Log('First buying' + ', Assets' + str(self['assets'][exchange]['ETH']) + ', CCI:' + str(current_cci))
            self.last_type = 'buy'
        ...
        # CCI從超買區返回正常區
        if self.last_cci > 100 and current_cci <= 100 and cur_cross == self.DOWN and (close_price - self.last_buy_price) >=  0.10 * close_price and not self.cci_sell:
            Log('CCI selling' + ', Assets' + str(self['assets'][exchange]['ETH'])+ ', CCI:' + str(current_cci) +  ',last CCI:' + str(self.last_cci))
            self.last_type = 'sell'
            self.last_cross_status = cur_cross
        ...
        # CCI從超賣區返回正常區
        elif self.last_cci < -100 and current_cci >= -100 and cur_cross == self.UP and (close_price - self.last_sell_price) <=  -0.10 * close_price and not self.cci_buy:
            Log('CCI buying' + ', Assets' + str(self['assets'][exchange]['ETH'])+ ', CCI:' + str(current_cci)+  ',last CCI:' + str(self.last_cci))
            self.last_type = 'buy'
            self.last_cross_status = cur_cross
            self.last_buy_price = close_price
            self.last_cci = current_cci
        ...
        # CCI 在超買或超賣區 不做事
            ...
            return []
        # CCI在正常區域
        elif current_cci > -100 and current_cci < 100:
            if self.last_type == 'sell' and cur_cross == self.UP and self.last_cross_status == self.DOWN:
                Log('RSI buying' + ', Assets' + str(self['assets'][exchange]['ETH'])+ ', CCI:' + str(current_cci))
                self.last_type = 'buy'
                self.last_cross_status = cur_cross
                self.last_buy_price = close_price
                self.last_cci = current_cci
            ...
            # cross down
                ...
                Log('RSI selling' + ', Assets' + str(self['assets'][exchange]['ETH'])+ ', CCI:' + str(current_cci))
                self.last_type = 'sell'
                self.last_cross_status = cur_cross
                self.last_sell_price = close_price
                self.last_cci = current_cci
